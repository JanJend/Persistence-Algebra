...
std::vector<ModuleMorphism<Matrix>> module_hom_space_basis(
    std::shared_ptr<const PersistenceModule<Matrix>> domain,
    std::shared_ptr<const PersistenceModule<Matrix>> target,
    ...
    // Module morphisms are equivalence classes of lifts, so remove maps that
    ...
    std::vector<ModuleMorphism<Matrix>> result;
...
template <typename Matrix>
std::vector<ModuleMorphism<Matrix>> module_endomorphism_basis(
    std::shared_ptr<const PersistenceModule<Matrix>> module,
    bool use_hom_exactness = false, bool info = false) {
