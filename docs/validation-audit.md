...
| `submodule.hpp`: constructor, lazy/exact generator reduction, containment, presentation construction | Automatic structural scans and certificate repair are diagnostic-only. Removed duplicate dimension/grading checks inside explicit `validate`, which is now public. |
...
| `hom_operations.hpp`: constructors and `validate` | Constructors remain trusted and scan-free. Explicit `validate` checks structure without repairing sorting flags. `check_lifts` computes the equations when requested. |
| `hom_operations.hpp`: identity, zero, shifts | Identity inherits the relevant differential's sorting certificate. Zero maps between independent endpoints retain initial sorting discovery. General shifts' repeated grading checks are diagnostic-only; canonical shifts retain a single nonnegative-amount check. |
